...
notarize_dmg() {(
  ...
  uuid=`xcrun altool --notarize-app --primary-bundle-id "$MACOS_SIGN_IDENTITY" --username "$MACOS_NOTARIZE_USER" --password "$MACOS_NOTARIZE_PASS" --file "$BEAM_WALLET_UI_IN" 2>&1 | grep 'RequestUUID' | awk '{ print $3 }'`
